...
def fragmentar_y_clasificar_con_mascara(imagen_path, clasificadores, scaler, distancias, angulos, propiedades, num_caracteristicas_esperadas, carpetas, umbral_knn=0.8, umbral_svm=0.5):
    ...
    for i, fragmento in enumerate(fragmentos):
        ...
        if len(caracteristicas) == num_caracteristicas_esperadas:
            ...
            # Predicciones KNN con probabilidad
            knn_pred = clasificadores['knn'].predict(caracteristicas)[0]
            knn_prob = clasificadores['knn'].predict_proba(caracteristicas)[0]
            knn_conf = max(knn_prob)

            if knn_conf < umbral_knn:
                knn_pred = "Desconocido"

            # Predicciones SVM con probabilidad
            svm_pred = clasificadores['svm'].predict(caracteristicas)[0]
            svm_prob = clasificadores['svm'].predict_proba(caracteristicas)[0]
            svm_conf = max(svm_prob)

            if svm_conf < umbral_svm:
                svm_pred = "Desconocido"

            # Determinar la predicción final
            pred_final = svm_pred if svm_pred != "Desconocido" else knn_pred

            resultados.append({
                "Fragmento": i + 1,
                "KNN": f"{knn_pred} ({knn_conf * 100:.2f}%)",
                "SVM": f"{svm_pred} ({svm_conf * 100:.2f}%)",
                "Final": pred_final
            })
